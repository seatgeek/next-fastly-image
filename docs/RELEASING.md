...
How a version of `@seatgeek/next-fastly-image` gets to npm. The short version: changesets accumulate on `main`, a maintainer versions them, and **pushing a `v<version>` tag triggers the publish** - still gated by guards and a human environment approval. Pushing commits to `main` never publishes.
...
2. **The `npm-publish` GitHub Environment** - create it explicitly under *Settings → Environments* and add **required reviewers**. This is the human approval gate.
   ...
   > [!WARNING]
   > If the workflow runs while the environment doesn't exist, GitHub silently auto-creates it **with no protection rules** - the approval gate would be hollow. Create it, with reviewers, before the first dispatch.
...
3. **`NPM_TOKEN`** - store it as a secret **on the `npm-publish` environment**, not at repo level. A repo-level token works mechanically but bypasses the reviewer gate's purpose; if one was added at repo level, move it and delete the repo-level copy.
   ...
   The token must be a **granular access token** scoped to the package (read/write, short expiry). A classic "Publish" token fails in CI with `EOTP` when the account requires 2FA for writes - CI cannot answer an OTP prompt.
...
3. **CI green on `main`.** The `ci` workflow must pass on the release commit.
4. **Smoke test.** Run the manual Next.js integration check from [CONTRIBUTING.md](./CONTRIBUTING.md#manual-smoke-test-nextjs-integration) against the packed tarball.
5. **Tag to release.** Tag the release commit on `main` and push the tag - this triggers the `release` workflow:
   ...
   The tag must be exactly `v<package.json version>` - a mismatch fails the guard. The workflow re-runs every quality gate, pauses for an `npm-publish` environment reviewer, publishes to npm with provenance, then **creates the GitHub Release automatically** with the version's CHANGELOG section as notes. (Fallback: a manual *Actions → release → Run workflow* dispatch from `main` also works, e.g. to retry after a transient failure - it skips the GitHub Release step since there is no tag ref.)
...
- Verify the npm, bundlephobia, and packagephobia badges in the README render (they 404 until the package exists on npm).
- Relative `./docs/...` links in the README are rewritten by npm against the `repository` field - spot-check them on the npm package page.
